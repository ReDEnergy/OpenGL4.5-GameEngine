...
PhysXRagdoll::PhysXRagdoll(PhysXManager * const pxManager)
	: pxManager(pxManager)
{
...
void PhysXRagdoll::Load(const string &fileName)
{
	...
	//PxAllocatorCallback;
	PxStringTable *stringTable = &PxStringTableExt::createStringTable(pxManager->gAllocator);
	PxSerializationRegistry* registry = PxSerialization::createSerializationRegistry(*pxManager->gPhysics);

	PxDefaultMemoryInputData sharedInputStream(data, fileSize);
	PxCollection* collection = PxSerialization::createCollectionFromXml(sharedInputStream, *pxManager->gCooking, *registry, 0, stringTable);
	...
	pxManager->gScene->addCollection(*collection);

	createChain2(pxManager->gPhysics, pxManager->gScene, PxTransform(PxVec3(0.0f, 40.0f, 20.0f)), 5, PxBoxGeometry(2.0f, 0.5f, 0.5f), 6.0f, createDampedD62);
...
void PhysXRagdoll::Update(float deltaTime, Camera *camera)
{
	if (dynamicSphere) {
		...
		createDynamic(pxManager->gPhysics, pxManager->gScene, PxTransform(origin), PxSphereGeometry(10), 200 * velocity);
	}
}
...
void PhysXRagdoll::Update(float deltaTime, int mods)
{
	...
	if (InputSystem::KeyHold(GLFW_KEY_L)) {
	...
	if (InputSystem::KeyHold(GLFW_KEY_I) && mods == GLFW_MOD_SHIFT) {
...
void PhysXRagdoll::OnMouseBtnEvent(int mouseX, int mouseY, int button, int action, int mods)
{
